...
class BioMarginalityReport(models.Model):
    _name = "bio.marginality.report"
    _description = "Marginality Report"
    _auto = False
    _rec_name = 'date'
    _order = 'date desc'
    ...
    def _with_marginality(self):
        return ""

    def _select_marginality(self):
        select_ = f"""
            MIN(l.id) AS id,
    ...
    def _from_marginality(self):
        return """
            ...
            """.format(currency_table=self.env['res.currency']._get_query_currency_table({'multi_company': True,
                                                                                          'date': {'date_to': fields.Date.today()}
                                                                                          }),
                       )

    def _where_marginality(self):
        return """
            l.display_type IS NULL"""

    def _group_by_marginality(self):
        return """
            l.product_id,
            l.order_id,
            t.uom_id,
    ...
    def _query(self):
        with_ = self._with_marginality()
        return f"""
            {"WITH" + with_ + "(" if with_ else ""}
            SELECT {self._select_marginality()}
            FROM {self._from_marginality()}
            WHERE {self._where_marginality()}
            GROUP BY {self._group_by_marginality()}
            {")" if with_ else ""}
        """
